...
void tensor_norm_test()
{
    ...
    for(int j=0; j<2; j++)
    {
        for(int i=0; i<3; i++)
        {
            if( std::abs( tensor_b(i,j)-norm( tensor_a(i,j) ) ) >1e-12 ) flag++;
        }
    }
    if(flag==0) cout<<"PASSED! Tensor norm passed complex<double> test!"<<endl;
    else cout<<"WARNING!!!!Tensor norm failed complex<double> test!"<<endl;
